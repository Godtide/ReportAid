...
export const reducer = (state: PayloadProps = initialState, action: ActionProps): PayloadProps => {
  ...
  switch (action.type) {
    ...
    case IATIReportActionTypes.ACTIVITYTRANSACTION_SUCCESS:
    case IATIReportActionTypes.ACTIVITYRELATEDACTIVITY_SUCCESS: {
      const data = (action.payload.data as PayloadProps)
      return {...state, ...data}
    }
    ...
    case IATIReportActionTypes.ACTIVITYTRANSACTION_FAILURE:
    case IATIReportActionTypes.ACTIVITYRELATEDACTIVITY_FAILURE:
    default:
      return state
  }
}
